...
- State persistence:
  ...
  - Input is sanitized to a fixed schema: progress, notes, solutions, activity, settings.

Key behaviors:
- Requires valid session for state operations.
- Creates the dsa_tracker_state table on first use if missing.
...
- Only top-level keys progress, notes, solutions, activity, settings are preserved.
...
Method --> |GET| Read["SELECT state, updated_at WHERE id=1"]
...
**Section sources**
- [state.js:4-21](file://api/state.js#L4-L21)
- [state.js:23-49](file://api/state.js#L23-L49)
...
- Operations:
  - CREATE TABLE IF NOT EXISTS on every request to ensure schema availability.
  - SELECT for GET requests.
  - INSERT ... ON CONFLICT (id) DO UPDATE for POST requests to upsert the single row.

**Section sources**
- [state.js:29-44](file://api/state.js#L29-L44)
...
**Section sources**
- [state.js:30-44](file://api/state.js#L30-L44)
...
- Cold starts:
  - First request may incur table creation overhead; subsequent calls reuse the existing table.
...
- POST /api/state
  - Request: { state: { progress, notes, solutions, activity, settings } }
