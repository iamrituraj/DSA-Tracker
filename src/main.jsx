...
function App(){
 ...
 const markProblem=(id,status)=>{update(id,{status});recordActivity()};
 ...
 return <div className="app">
  ...
  <main><header><div><h1>{page==="dashboard"?"Dashboard":page==="roadmap"?"A2Z Roadmap":page==="problem"?"Problem":page[0].toUpperCase()+page.slice(1)}</h1><p>Practice, track, revise, master.</p></div><div className="search"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search problems, topics, patterns…"/></div></header>
   ...
   {page==="problem"&&selected&&<Problem p={selected} progress={progress[selected.id]||{}} update={update} notes={notes[selected.id]||{}} setNotes={setNotes} back={()=>setPage("roadmap")} recordActivity={recordActivity} setToast={setToast}/>} 
  </main>
...
function Patterns({problems,open}){const map={};problems.forEach(p=>(map[p.pattern]??=[]).push(p));return <section><div className="pattern-grid">{Object.entries(map).map(([pat,ps])=>{const solved=ps.filter(p=>p.status==="Solved"||p.status==="Mastered").length;return <div className="pattern-card" key={pat}><div className="pattern-top"><div><span className="pattern-icon">◆</span><h3>{pat}</h3><p>{solved}/{ps.length} completed</p></div><strong>{ps.length?Math.round(solved/ps.length*100):0}%</strong></div><div className="bar"><i style={{width:`${ps.length?solved/ps.length*100:0}%`}}/></div>{ps.slice(0,5).map(p=><button key={p.id} onClick={()=>open(p)}><span>{p.title}</span><span className={`mini-status ${p.status?.toLowerCase().replace(" ","-")}`}>{p.status}</span></button>)}</div>})}</div></section>}
...
function Problem({p,progress,update,notes,setNotes,back,recordActivity,setToast}){
 ...
 const mark=(status)=>{const nextStatus=status;const count=progress.revisionCount||0;const firstStep=revisionSteps[Math.min(count,revisionSteps.length-1)];update(p.id,{status:nextStatus,lastRevised:new Date().toISOString(),nextRevision:nextStatus==="Not Started"?null:addDays(firstStep),revisionCount:count,attempts:(progress.attempts||0)+(status!=="Not Started"?1:0),confidence:nextStatus==="Mastered"?"🔵 Interview Ready":progress.confidence||"🟡 Learning"});recordActivity();setToast(`${status} saved.`)};
 const changeConfidence=e=>update(p.id,{confidence:e.target.value});
 const scheduleRevision=()=>{const count=progress.revisionCount||0;const step=revisionSteps[Math.min(count,revisionSteps.length-1)];update(p.id,{nextRevision:addDays(step),lastRevised:new Date().toISOString(),revisionCount:count});setToast(`Next revision in ${step} day${step!==1?"s":""}.`)};
 return <section><button className="back" onClick={back}>← Back to roadmap</button><div className="problem-header"><div><span className="eyebrow">{p.topic} · {p.pattern}</span><h2>{p.title}</h2><div className="meta"><span className={`diff ${p.difficulty.toLowerCase()}`}>{p.difficulty}</span><a href={p.url} target="_blank" rel="noreferrer">Open problem <ExternalLink size={13}/></a></div></div><div className="actions"><button className={progress.favorite?"fav active":"fav"} onClick={()=>update(p.id,{favorite:!progress.favorite})}><Star size={15} fill={progress.favorite?"currentColor":"none"}/></button><button onClick={()=>mark("Attempted")}>Attempted</button><button className="primary" onClick={()=>mark("Solved")}><CheckCircle2 size={16}/>Solved</button><button onClick={()=>mark("Mastered")}>Mastered</button></div></div>
 <div className="detail-grid"><div><div className="panel"><div className="field"><label>Confidence</label><select value={progress.confidence||"🟡 Learning"} onChange={changeConfidence}>{confidence.map(c=><option key={c}>{c}</option>)}</select></div><div className="mini-stats"><span>Status<b>{progress.status||"Not Started"}</b></span><span>Attempts<b>{progress.attempts||0}</b></span><span>Revisions<b>{progress.revisionCount||0}</b></span></div><div className="problem-tools"><button onClick={scheduleRevision}><CalendarDays size={15}/>Schedule next revision</button><span>{progress.nextRevision?`Next: ${new Date(progress.nextRevision).toLocaleDateString()}`:"No revision scheduled"}</span></div></div><div className="panel"><h3>Learning notes</h3><label>Key insight</label><textarea value={local.insight||""} onChange={e=>setLocal({...local,insight:e.target.value})} placeholder="What is the key idea?"/><label>My mistake</label><textarea value={local.mistake||""} onChange={e=>setLocal({...local,mistake:e.target.value})} placeholder="What did you miss or get wrong?"/><label>Approach</label><textarea value={local.approach||""} onChange={e=>setLocal({...local,approach:e.target.value})} placeholder="Write the approach in your own words."/><label>Complexity</label><textarea className="short" value={local.complexity||""} onChange={e=>setLocal({...local,complexity:e.target.value})} placeholder="Time: O(?)  Space: O(?)"/><label>Interview cue</label><textarea className="short" value={local.interviewCue||""} onChange={e=>setLocal({...local,interviewCue:e.target.value})} placeholder="What should trigger this pattern in an interview?"/><button className="primary save" onClick={saveNotes}>Save notes</button></div></div><div><div className="panel challenge"><Brain size={22}/><h3>Revision ladder</h3><p>Each review moves the problem farther into the future. Reconstruct the solution before looking at your notes.</p><div className="revision-steps">{revisionSteps.map((d,i)=><span className={(progress.revisionCount||0)>i?"done-step":""} key={d}>{d} day{d!==1?"s":""}</span>)}</div></div><div className="panel"><h3>Problem metadata</h3><div className="metadata"><span>Topic<b>{p.topic}</b></span><span>Pattern<b>{p.pattern}</b></span><span>Difficulty<b>{p.difficulty}</b></span><span>Last revised<b>{progress.lastRevised?new Date(progress.lastRevised).toLocaleDateString():"Never"}</b></span></div></div><div className="panel checklist"><h3>Before marking mastered</h3><p><CheckCircle2 size={14}/>Can explain the pattern?</p><p><CheckCircle2 size={14}/>Can solve without looking?</p><p><CheckCircle2 size={14}/>Know time & space complexity?</p><p><CheckCircle2 size={14}/>Know when to use this pattern?</p></div></div></div></section>}
